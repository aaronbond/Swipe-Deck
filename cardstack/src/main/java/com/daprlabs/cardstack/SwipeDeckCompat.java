 ...
 */
public class SwipeDeckCompat extends FrameLayout {
    ...
    private SwipeListenerCompat swipeListener;
    private View topCard;
    private int leftImageResource;
    private int rightImageResource;
    private boolean topCardSettled = false;

    public SwipeDeckCompat(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void addNextCard() {
        if (nextAdapterCard < mAdapter.getCount()) {
            ...
            //ViewCompat.setY(newBottomChild,paddingTop);
    ...
    private void addAndMeasureChild(View child) {
        ...
        for (View c : children) {
            ...
            if (leftImageResource != 0){
                View left = child.findViewById(leftImageResource);
                ObjectAnimator.ofFloat(left, "alpha", 0).start();
            }
            if (rightImageResource != 0){
                View right = child.findViewById(rightImageResource);
                ObjectAnimator.ofFloat(right, "alpha", 0).start();
            }
        }
        setZTranslations();
    }
    ...
    private void positionItem(int index) {
        ...
        ObjectAnimator.ofFloat(child, "translationY", paddingTop + offset);
    ...
    private void setupTopCard() {
        ...
        if (child != null) {
            ...
            swipeListener = new SwipeListenerCompat(child, new SwipeListenerCompat.SwipeCallback() {
                @Override
    ...
    public void swipeTopCardLeft(int duration) {
        ...
        if(childCount > 0 && topCardSettled){
            ...
            swipeListener.animateOffScreenLeft(getChildAt(getChildCount() - 1), duration);
            //needs to check card for event, after callback
        }
    }
    ...
    public void swipeTopCardRight(int duration) {
        ...
        if(childCount > 0 && topCardSettled){
            topCardSettled = false;
            swipeListener.animateOffScreenRight(getChildAt(getChildCount() - 1), duration);
            //needs to check card for event create a callback
        }
    }
